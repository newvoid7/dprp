...
def convert_to_pns():
    ...
    for fold in range(4):
        fold_kidney_dir = os.path.join(target_dir, f'fold{fold}_kidney')
        fold_tumor_dir = os.path.join(target_dir, f'fold{fold}_tumor')
        ...
        for i, img in enumerate(images):
            cv2.imwrite(os.path.join(fold_kidney_dir, 'Frame', f'{i}.png'), img)
            cv2.imwrite(os.path.join(fold_tumor_dir, 'Frame', f'{i}.png'), img)
        for i, lbl in enumerate(labels):
            ch_label = characterize(lbl.transpose((2, 0, 1)), LABEL_GT_CHARACTERIZER)
            cv2.imwrite(os.path.join(fold_kidney_dir, 'GT', f'{i}.png'), ch_label[0] * 255)
            cv2.imwrite(os.path.join(fold_tumor_dir, 'GT', f'{i}.png'), ch_label[1] * 255)
